...
class UiActionsBridge(QObject):
    commandSubmitted = Signal(str)
    stopRequested = Signal()
    liveModeRequested = Signal(bool)
    liveVoiceRequested = Signal(bool)
    modeSelected = Signal(str)
    visionSelected = Signal(str)
    agentViewToggleRequested = Signal()
    backgroundVisibilityToggleRequested = Signal()
    minimizeRequested = Signal()
    restoreRequested = Signal()
    expandToggleRequested = Signal()
    clickThroughToggleRequested = Signal()
    ...
    def submitCommand(self, text: str) -> None:
        clean = str(text or "").strip()
        if clean:
            self.commandSubmitted.emit(clean)
    ...
    @Slot(bool)
    def requestLiveMode(self, enabled: bool) -> None:
        self.liveModeRequested.emit(bool(enabled))

    @Slot(bool)
    def requestLiveVoice(self, enabled: bool) -> None:
        self.liveVoiceRequested.emit(bool(enabled))

    @Slot(str)
    def selectMode(self, value: str) -> None:
        self.modeSelected.emit(_normalize_mode(value))

    @Slot(str)
    def selectVision(self, value: str) -> None:
        self.visionSelected.emit(_normalize_vision(value))

    @Slot()
    def toggleAgentView(self) -> None:
        self.agentViewToggleRequested.emit()

    @Slot()
    def toggleBackgroundVisibility(self) -> None:
        self.backgroundVisibilityToggleRequested.emit()
    ...
    def toggleClickThrough(self) -> None:
        self.clickThroughToggleRequested.emit()
