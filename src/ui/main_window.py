...
class MainWindow(QMainWindow):
    ...
    def toggle_minimize(self):
        if not self.minimized:
            ...
            self.chat_widget.chat_display.hide()
            self.chat_widget.input_frame.hide()
            self.chat_widget.mode_combo.hide()
            self.chat_widget.expand_btn.hide()
            self.chat_widget.minimize_btn.setText("+")
            self.minimized = True
        else:
            self.setFixedSize(*self.last_size)
            self.chat_widget.chat_display.show()
            self.chat_widget.input_frame.show()
            self.chat_widget.mode_combo.show()
